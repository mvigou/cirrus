...
/* ### MVC classes ### */
	...
	class PubController {
		...
		static barInput = () => {
			if(ui.nav.leftCt.barFormInp.value.indexOf('+') === 0) {
				ui.nav.leftCt.barFormBt.classList.remove('--hidden');
			}
			else {
				ui.nav.leftCt.barFormBt.classList.add('--hidden');
				const items = document.getElementsByTagName('li');
				for(const item of items) {
					item.classList.remove('--visible');	
					if(item.textContent.toLowerCase().indexOf(ui.nav.leftCt.barFormInp.value.substr(1).toLowerCase()) >= 0) {
		...
		static barSubmit = e => {
			if(ui.nav.leftCt.barFormInp.value.indexOf('+') === 0) {
				if(ui.nav.leftCt.barFormInp.value.length > 1) {
					PubController.createDirectory(ui.nav.leftCt.barFormInp.value.substr(1));
					ui.nav.leftCt.barFormBt.classList.add('--hidden');
					ui.nav.leftCt.barFormInp.value = '';
				}
			}
			e.preventDefault();
		};
...
/* ### build user interface ### */
	...
	/* main nav */
		...
		ui.nav.leftCt.barFormLab = document.createElement('label');
		ui.nav.leftCt.barFormLab.setAttribute('for', 'bar__input');
		ui.nav.leftCt.barFormLab.textContent = 'Rechercher ou créer un dossier en commençant par +';
		ui.nav.leftCt.barForm.appendChild(ui.nav.leftCt.barFormLab);

		ui.nav.leftCt.barFormInp = document.createElement('input');
		ui.nav.leftCt.barFormInp.setAttribute('for', 'bar__input');
		ui.nav.leftCt.barFormInp.setAttribute('placeholder', 'chercher / créer dossier avec +');
		ui.nav.leftCt.barFormInp.textContent = 'Rechercher ou créer un dossier en commençant par +.';
		ui.nav.leftCt.barFormInp.oninput = () => PubController.barInput();
		ui.nav.leftCt.barForm.appendChild(ui.nav.leftCt.barFormInp);

		ui.nav.leftCt.barFormBt = document.createElement('button');
		ui.nav.leftCt.barFormBt.setAttribute('class', '--hidden');
		ui.nav.leftCt.barFormBt.setAttribute('title', 'Créer le dossier');
		ui.nav.leftCt.barFormBt.innerHTML = '<svg viewBox="3 3 18 18"><path d="M12.75 16H11.25V13.75H9V12.25H11.25V10H12.75V12.25H15V13.75H12.75V16Z" /><path fill-rule="evenodd" clip-rule="evenodd" d="M19.3636 6.63636C20.2674 6.63636 21 7.36899 21 8.27273V18.0909C21 18.9946 20.2674 19.7273 19.3636 19.7273H4.63636C3.73262 19.7273 3 18.9946 3 18.0909V6.63636C3 5.73262 3.73262 5 4.63636 5H9.54545C10.4618 5 11.0451 5.38901 11.631 6.12815C11.6472 6.14858 11.6866 6.19938 11.7303 6.25567C11.792 6.3352 11.8624 6.42596 11.8875 6.45703C11.9129 6.48848 11.9342 6.51449 11.9519 6.53598C12.0239 6.6235 12.0343 6.63619 12.0015 6.63636H19.3636ZM19.3636 18.0909V8.27273L11.9956 8.27272C11.408 8.26957 11.0253 7.99447 10.6139 7.48444C10.5766 7.43825 10.4861 7.32155 10.4203 7.23679C10.3859 7.19243 10.3582 7.15674 10.3486 7.14458C10.0421 6.75792 9.85984 6.63636 9.54545 6.63636H4.63636V18.0909H19.3636Z" /></svg>';
		ui.nav.leftCt.barForm.appendChild(ui.nav.leftCt.barFormBt);
