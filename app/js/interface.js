	...
	chess(
		{
			...
			attributes: {
				class: 'bwr__nav__bt datas-feature',
				onclick: 'toggleActive(UI.createDirForm), UI.createDirForm.querySelector(\'input\').focus()',
	...
	chess(
		{
			...
			attributes: {
				class: 'bwr__nav__bt recycle-feature dangerous',
...
UI.pop = chess(
	{ type: 'div', attributes: { class: 'pop prg-click' }, children: [
		{ type: 'div', children: [
			...
			{ type: 'div', attributes: { class: 'pop__mess' }}
		]}
	]}
);
...
UI.createDirForm = chess(
	{ 
		type: 'form', 
		attributes: { class: 'create-dir-form'}, 
		children: [
			{ type: 'input', attributes: { type: 'text' }},
			{ type: 'button', attributes: { title: lab.bt.add, type: 'submit' }, text: '+' }	
		],
		events: [
			{
				type: 'submit',
...
UI.browserNav.appendChild(UI.createDirForm);
