...
function buildItem($item, $type, $dir) {
	...
	switch($type) {
		case 'file':
			$pathInAppContext = $dir . '/' . $item;
			$pathOutAppContext = $dir . '/' . $item;
			break;
		case 'subfolder':
			$pathInAppContext = '?dir=' . urlencode($dir . '/' . $item);
			$pathOutAppContext = $dir . '/' . $item;
			break;
		case 'parent':
			$pathInAppContext = '?dir=' . urlencode(substr($dir, 0, strrpos($dir, '/')));
			$pathOutAppContext = $dir . '/' . $item;
			break;
		default:
			$pathInAppContext = '';
	...
	echo // Common parts for parent, files and subfolders.
	'<div class="item">' .
		'<a class="item__link" href="' . $pathInAppContext . '">' .
			'<svg class="item__link__svg" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">';
			...
			'</svg>' .
			'<div>' .
				'<h3 class="item__link__title">' . $item . '</h3>';
				// Files only.
				if($type === 'file') {
					echo '<p class="item__link__info">Dernière modification le ' . date('Y-m-d à H:i:s', filemtime($pathOutAppContext)) . '</p>';
				}
			echo
			'</div>' .
		'</a>';
		...
		if($type === 'file' || $type === 'subfolder') {
			echo 
			'<div class="item__actions">' .
				'<a class="item__actions_a" href="' . $pathOutAppContext . '" title="Télécharger" download>' .
					'<svg class="item__actions__svg" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">';
					...
					echo // Common parts for files and subfolders. 
					'</svg>' .
				'</a>' .
				'<a class="item__actions_a" href="#" title="Supprimer">' .
					'<svg class="item__actions__svg" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">';
